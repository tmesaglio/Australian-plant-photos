...
#seperate the states
sep_state_data<-str_split(unique(apc_species$taxonDistribution),",")
...
state_parse_and_add_column<-function(species_df,state,apc_species){
  print(all_codes[grepl(state,all_codes)])
  species_df[,state]<-case_when(
    ...
    grepl(state, apc_species$taxonDistribution) ~ "native", #no entry = native, it's impotant this is last in the list
    TRUE ~ "not present"
  )
  return(species_df)
}
